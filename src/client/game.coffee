chart_progress = (svg_id, bugs) ->
  ...
  if bugs.plots?
    ...
    xScale     = new Plottable.Scale.Linear().domain([0, bugs.max_turns])
    yScale     = new Plottable.Scale.Linear().domain([0, bugs.count])
...
next_turn = (bugs, canvas) ->
  ...
  if bugs.turns % 10 == 0
    chart_progress '#jitterbug_progress_chart_svg', bugs
